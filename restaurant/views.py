...
class CartViewOperations(generics.ListCreateAPIView):
    ...
    def post(self, request, *arg, **kwargs):
        ...
        try:
            yourCart.objects.create(user=request.user, quantity=quantity, unit_price=item.price, price=price, menuitem_id=id)
        except:
    ...
    def delete(self, request, *arg, **kwargs):
        if request.data['menuitem']:
            ...
            cart = get_object_or_404(yourCart, user=request.user, menuitem=menuitem )
            cart.delete()
...
class OrderViewOperations(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        order = Orderlist.objects.create(user=request.user, status=False, total=total, date=date.today())
        for i in cart.values():
            menuitem = get_object_or_404(MenuItem, id=i['menuitem_id'])
            orderitem = OrderMenuitem.objects.create(order=order, menuitem=menuitem, quantity=i['quantity'])
            orderitem.save()
        cart.delete()
...
class ViewSingleOrder(generics.ListCreateAPIView):
    ...
    def get_permissions(self):
        ...
        if self.request.user == order.user and self.request.method == 'GET':
            ...
            permission_classes = [IsAuthenticated, IsDeliveryStaff | IsManager | IsAdminUser]
        return[permission() for permission in permission_classes] 
